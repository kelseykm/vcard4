...
describe("VCARD tests", () => {
  it("Accepts valid input", () => {
    assert.doesNotThrow(
      () => new VCARD([new FNProperty([], new TextType("James Bond"))])
    );
  ...
  it("Rejects invalid input", () => {
    ...
    assert.throws(
      () =>
        new VCARD([
          new FNProperty([], new TextType("James Bond")),
          new UIDProperty([], new TextType("blahblah")),
          new UIDProperty([], new TextType("mary had a little lamb")),
        ])
    );
    assert.throws(
      () =>
    ...
    assert.throws(
      () => new VCARD([new NoteProperty([], new TextType("James Bond"))])
    );
    assert.throws(
      () =>
        new VCARD([
          new FNProperty([], new TextType("James Bond")),
          new MemberProperty([], new URIType("uuid:blah-blah-blah")),
        ])
    );
  ...
  it("Formats value properly", () => {
    assert.strictEqual(
      new VCARD([new FNProperty([], new TextType("James Bond"))]).repr(),
      "BEGIN:VCARD\r\n" +
        "VERSION:4.0\r\n" +
        "FN:James Bond\r\n" +
        "END:VCARD\r\n"
    ...
    assert.strictEqual(
      new VCARD([new FNProperty([], new TextType("James Bond"))]).reprXML(),
    ...
    assert.deepEqual(
      new VCARD([new FNProperty([], new TextType("James Bond"))]).reprJSON(),
