...
describe("FNProperty tests", () => {
  it("Accepts valid input", () => {
    assert.doesNotThrow(() => new FNProperty([], new TextType("James Bond")));
  });

  it("Rejects invalid input", () => {
    assert.throws(() => new FNProperty([], new IntegerType(55)));
    assert.throws(
      () =>
        new FNProperty(
  ...
  it("Formats value properly", () => {
    assert.strictEqual(
      new FNProperty([], new TextType("James Bond")).repr(),
      "FN:James Bond"
    );
    assert.strictEqual(
      new FNProperty([], new TextType("James Bond")).reprXML(),
      "<fn><text>James Bond</text></fn>"
    );
    assert.deepEqual(
      new FNProperty([], new TextType("James Bond")).reprJSON(),
      ["fn", {}, "text", "James Bond"]
    );
  });
});
