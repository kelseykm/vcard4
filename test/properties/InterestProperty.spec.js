...
describe("InterestProperty tests", () => {
  it("Accepts valid input", () => {
    assert.doesNotThrow(() => new InterestProperty([], new TextType("golf")));
  });
  ...
  it("Rejects invalid input", () => {
    ...
    assert.throws(
      () => new InterestProperty([], new DateTimeType("19960415", "date"))
    );
  ...
  it("Formats value properly", () => {
    assert.strictEqual(
      new InterestProperty([], new TextType("golf")).repr(),
      "INTEREST:golf"
    );
    assert.strictEqual(
      new InterestProperty([], new TextType("golf")).reprXML(),
      "<interest><text>golf</text></interest>"
    );
    assert.deepEqual(
      new InterestProperty([], new TextType("golf")).reprJSON(),
      ["interest", {}, "text", "golf"]
    );
  });
});
