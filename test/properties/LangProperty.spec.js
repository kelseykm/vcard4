...
describe("LangProperty tests", () => {
  it("Accepts valid input", () => {
    assert.doesNotThrow(() => new LangProperty([], new LanguageTagType("fr")));
  });

  it("Rejects invalid input", () => {
    assert.throws(() => new LangProperty([], new IntegerType(55)));
    assert.throws(
      () =>
        new LangProperty(
  ...
  it("Formats value properly", () => {
    assert.strictEqual(
      new LangProperty([], new LanguageTagType("fr")).repr(),
      "LANG:fr"
    );
    assert.strictEqual(
      new LangProperty([], new LanguageTagType("fr")).reprXML(),
      "<lang><language-tag>fr</language-tag></lang>"
    );
    assert.deepEqual(
      new LangProperty([], new LanguageTagType("fr")).reprJSON(),
