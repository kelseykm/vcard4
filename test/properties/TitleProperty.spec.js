...
describe("TitleProperty tests", () => {
  ...
  it("Rejects invalid input", () => {
    assert.throws(
      () => new TitleProperty([new IntegerType(55)], new IntegerType(55))
    );
  ...
  it("Formats value properly", () => {
    assert.strictEqual(
      new TitleProperty([], new TextType("Research Scientist")).repr(),
      "TITLE:Research Scientist"
    );
    assert.strictEqual(
      new TitleProperty([], new TextType("Research Scientist")).reprXML(),
      "<title><text>Research Scientist</text></title>"
    );
    assert.deepEqual(
      new TitleProperty([], new TextType("Research Scientist")).reprJSON(),
