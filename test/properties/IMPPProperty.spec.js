...
describe("IMPPProperty tests", () => {
  ...
  it("Rejects invalid input", () => {
    assert.throws(() => new IMPPProperty([], new IntegerType(55)));
    assert.throws(
      () =>
        new IMPPProperty(
  ...
  it("Formats value properly", () => {
    assert.strictEqual(
      new IMPPProperty([], new URIType("xmpp:alice@example.com")).repr(),
      "IMPP:xmpp:alice@example.com"
    );
    assert.strictEqual(
      new IMPPProperty([], new URIType("xmpp:alice@example.com")).reprXML(),
      "<impp><uri>xmpp:alice@example.com</uri></impp>"
    );
    assert.deepEqual(
      new IMPPProperty([], new URIType("xmpp:alice@example.com")).reprJSON(),
