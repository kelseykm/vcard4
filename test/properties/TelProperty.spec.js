...
describe("TelProperty tests", () => {
  it("Accepts valid input", () => {
    ...
    assert.doesNotThrow(
      () => new TelProperty([], new TextType("+33-01-23-45-67"))
    );
  });

  it("Rejects invalid input", () => {
    assert.throws(() => new TelProperty([], new IntegerType(55)));
    assert.throws(
      () =>
        new TelProperty(
  ...
  it("Formats value properly", () => {
    assert.strictEqual(
      new TelProperty([], new URIType("tel:+33-01-23-45-67")).repr(),
      "TEL:tel:+33-01-23-45-67"
    );
    assert.strictEqual(
      new TelProperty([], new URIType("tel:+33-01-23-45-67")).reprXML(),
      "<tel><uri>tel:+33-01-23-45-67</uri></tel>"
    );
    assert.deepEqual(
      new TelProperty([], new URIType("tel:+33-01-23-45-67")).reprJSON(),
