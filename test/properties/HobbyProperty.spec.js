...
describe("HobbyProperty tests", () => {
  it("Accepts valid input", () => {
    assert.doesNotThrow(() => new HobbyProperty([], new TextType("golf")));
  });
  ...
  it("Rejects invalid input", () => {
    ...
    assert.throws(
      () => new HobbyProperty([], new DateTimeType("19960415", "date"))
    );
  ...
  it("Formats value properly", () => {
    assert.strictEqual(
      new HobbyProperty([], new TextType("golf")).repr(),
      "HOBBY:golf"
    );
    assert.strictEqual(
      new HobbyProperty([], new TextType("golf")).reprXML(),
      "<hobby><text>golf</text></hobby>"
    );
    assert.deepEqual(new HobbyProperty([], new TextType("golf")).reprJSON(), [
      "hobby",
      {},
      "text",
      "golf",
