...
describe("PIDParameter tests", () => {
  it("Accepts valid input", () => {
    assert.doesNotThrow(() => new PIDParameter(new IntegerType(1)));

    assert.doesNotThrow(
      () => new PIDParameter([new IntegerType(3), new IntegerType(7)])
    );

    assert.doesNotThrow(
      () => new PIDParameter([[new IntegerType(1), new IntegerType(5)]])
    );
  ...
  it("Formats value properly", () => {
    assert.strictEqual(
      new PIDParameter([
        [new IntegerType(1), new IntegerType(7)],
        [new IntegerType(23), new IntegerType(24)],
      ]).repr(),
      "PID=1.7,23.24"
    );
    assert.strictEqual(
